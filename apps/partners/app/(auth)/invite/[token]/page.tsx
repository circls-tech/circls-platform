...
import { ROLE_INFO, formatRole } from '@/lib/roles';
...
export default function AcceptInvitePage() {
        ...
        {result.roleChanged ? (
          ...
          <>
            ...
            <p className="text-sm text-slate-600">
              ...
              <strong>{formatRole(result.role)}</strong>. There&apos;s nothing to accept — this
              invite has been closed out.
            </p>
          </>
        )}
        <Link
  ...
  const invitedAs = (
    <>
      ...
      <strong>{ROLE_INFO[meta.role].label}</strong>
      {meta.inviterEmail ? ` by ${meta.inviterEmail}` : ''}.{' '}
      <span className="text-slate-500">{ROLE_INFO[meta.role].description}</span>
    </>
  );
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-sm flex-col justify-center gap-6 p-6">
      ...
      {isLogin && (
        <Link href="/forgot-password" className="text-center text-sm text-brand-700 hover:underline">
          Forgot password?
        </Link>
      )}
      {error && <p className="text-sm text-red-600">{error}</p>}
