...
const DEFAULT_CONFIG: QrTicketConfig = {
  enabled: true,
  multiUse: false,
  maxScans: null,
    ...
    <div className="flex flex-col gap-3">
      <div className="flex flex-col gap-1">
        ...
        <p className="text-xs text-[#94a3b8]">
          When enabled, every finalised purchase issues scannable QR passes your staff validate on
          the Check-in page.
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 gap-3 rounded-[var(--radius)] border border-[#e5e7eb] bg-slate-50 p-3 sm:grid-cols-12">
            ...
            <div className="inline-flex w-fit rounded-md border border-slate-200 bg-white p-0.5">
              {([false, true] as const).map((multi) => (
                <button
                  ...
                  type="button"
                  onClick={() =>
                    patch({ multiUse: multi, ...(multi ? {} : { maxScans: null }) })
                  }
                  className={[
                    'rounded px-3 py-1.5 text-sm font-medium transition-colors',
                    cfg.multiUse === multi
          ...
          {cfg.multiUse && (
            <div className="sm:col-span-4">
              <Input
                ...
                onChange={(e) => patch({ maxScans: parseIntOrNull(e.target.value) })}
          ...
          <div className={cfg.multiUse ? 'sm:col-span-4' : 'sm:col-span-6'}>
            <Input
              ...
              onChange={(e) => patch({ validFromOffsetMin: parseIntOrNull(e.target.value) })}
          ...
          <div className={cfg.multiUse ? 'sm:col-span-4' : 'sm:col-span-6'}>
            <Input
              ...
              onChange={(e) => patch({ validUntilOffsetMin: parseIntOrNull(e.target.value) })}
