...
/**
 * Event photo gallery + uploader for the partner portal. Same flow as
 * VenueImages: presigned PUT straight to R2, public-URL reads.
 */
    ...
    <section className="flex flex-col gap-3 rounded border border-gray-200 bg-white p-4">
      ...
      {!isLoading && count === 0 && (
        <p className="text-sm text-gray-500">
          No photos yet. The first photo becomes the event’s cover image.
        </p>
      )}
        ...
        <ul className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4">
          {images!.map((img, i) => (
            ...
            >
              ...
              <img src={img.url} alt="Event photo" className="h-full w-full object-cover" loading="lazy" />
              {i === 0 && (
      ...
      <ConfirmDialog
        open={confirmId !== null}
        title="Delete photo?"
        message="This removes the photo from the event and from storage. This can't be undone."
        confirmLabel="Delete"
        danger
        onConfirm={() => {
          if (confirmId) {
