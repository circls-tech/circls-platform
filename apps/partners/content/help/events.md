...
**Finished events archive themselves.** Once an event has been over for a full day, circls moves it to **Archived** for you — it checks twice a day, at 5 am and 5 pm IST, so this happens within a day and a half of the end time. A live event that has run is marked **ended** on the way. Events awaiting review are left alone. If you **Restore** one, it stays restored — circls only ever archives an event once.
