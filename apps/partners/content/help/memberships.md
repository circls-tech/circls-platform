...
Signed someone up at the desk, over the phone, or on paper? Click **Reception** beside the plan's name — or **Add member** in that list; both open the same form, and Reception is there so counter staff can reach it in one click. It is available only while the plan is active. You give their name, an optional contact, which tier they are on, and — if you want something other than the tier's normal run — the dates their membership covers.
