...
import { useAdminCoupons, useAdminCouponStats, useUpdateAdminCoupon } from '@/lib/api/queries';
      ...
      {stats && (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
            <p className="text-xs font-medium uppercase tracking-wide text-slate-500">Circls-funded discount spend</p>
            <p className="mt-1 text-2xl font-semibold tabular-nums text-slate-900">{rupees(stats.platformFunded.discountPaise)}</p>
            <p className="mt-0.5 text-xs text-slate-500">{stats.platformFunded.redemptions} redemption{stats.platformFunded.redemptions === 1 ? '' : 's'} — absorbed by Circls, partner payouts unaffected.</p>
          </div>
          <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
            <p className="text-xs font-medium uppercase tracking-wide text-slate-500">Partner-funded discounts</p>
            <p className="mt-1 text-2xl font-semibold tabular-nums text-slate-900">{rupees(stats.orgFunded.discountPaise)}</p>
            <p className="mt-0.5 text-xs text-slate-500">{stats.orgFunded.redemptions} redemption{stats.orgFunded.redemptions === 1 ? '' : 's'} — funded by partners via their own coupons.</p>
          </div>
        </div>
      )}
        ...
        <div className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {stats.monthly.map((m) => (
                <tr key={m.month}>
                  <td className="px-4 py-2 text-xs text-slate-700">{m.month}</td>
                  <td className="px-4 py-2 text-right text-xs tabular-nums text-slate-600">{m.redemptions}</td>
                  <td className="px-4 py-2 text-right text-xs tabular-nums text-slate-700">{rupees(m.discountPaise)}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            ...
            {isError && <tr><td colSpan={8} className="px-4 py-8 text-center text-red-600">{error instanceof Error ? error.message : 'Failed to load'}</td></tr>}
            {!isLoading && !isError && (coupons?.length ?? 0) === 0 && <tr><td colSpan={8} className="px-4 py-8 text-center text-slate-400">No platform coupons.</td></tr>}
            {coupons?.map((c) => (
              <tr key={c.id} className="hover:bg-slate-50">
                ...
                <td className="px-4 py-2.5 text-right text-xs tabular-nums text-slate-700">{statByCoupon.has(c.id) ? rupees(statByCoupon.get(c.id)!.discountPaise) : '—'}</td>
