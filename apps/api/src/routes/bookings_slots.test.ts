...
vi.mock('../lib/firebase_admin.js', () => ({
  verifyIdToken: vi.fn(async (token: string) => {
    const map: Record<string, Record<string, unknown>> = {
      owner: { uid: 'fbuid_bsowner', email: 'bsowner@x.com', email_verified: true },
      ownerB: { uid: 'fbuid_bsownerb', email: 'bsownerb@x.com', email_verified: true },
      other: { uid: 'fbuid_bsother', email: 'bsother@x.com', email_verified: true },
    };
    const u = map[token];
