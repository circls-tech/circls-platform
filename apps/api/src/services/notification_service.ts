...
/**
 * Called from `kyc_service` state transitions. Resolves the tenant owner
 * (`tenant_members WHERE role='owner'`) and emails them the appropriate
 * template. If the tenant has multiple owners we notify all of them.
 */
export async function notifyKycStateChange(
  ...
  let templateKey: 'kyc.verified' | 'kyc.rejected';
  if (newStatus === 'verified') templateKey = 'kyc.verified';
  else if (newStatus === 'rejected') templateKey = 'kyc.rejected';
  else return; // no template for in_review / submitted / pending
  ...
  const [tenant] = await db
    ...
    .limit(1);
  if (!tenant) {
    logger.warn({ tenantId }, 'notify_kyc_missing_tenant');
    return;
  }
  ...
  if (owners.length === 0) {
    logger.warn({ tenantId }, 'notify_kyc_no_owners');
    return;
  }

  const payload: Record<string, unknown> = {
    tenantName: tenant.name,
    reason: opts.rejectionReason ?? '(see Partner portal for details)',
  };

  for (const owner of owners) {
    if (!owner.email) continue; // can't email a phone-only user
    await safeDispatch({
      tenantId,
      userId: owner.userId,
      channel: 'email' as NotificationChannel,
